...
Terraform configuration for provisioning EC2 + nginx + Let's Encrypt SSL on AWS with Cloudflare DNS.

## File Structure

```
docs/c4flow/terraform/<feature-slug>/
├── .gitignore         ← generated by skill (see Step 3)
├── variables.tf       ← see below
├── main.tf            ← see below
├── outputs.tf         ← see below
└── terraform.tfvars   ← generated by skill with resolved values
```
